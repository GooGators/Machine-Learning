...
ipca = decomposition.FastICA(n_components=45, whiten=True)
ipcanew = ipca.fit_transform(X)

gm = mixture.GMM(n_components=2, covariance_type='tied')
gm.fit(ipcanew)

ypred = gm.predict(ipcanew)
...
for i in range(2, numPoints):
    ipc = decomposition.FastICA(n_components=45, whiten=True)
    icanew = ipc.fit_transform(X, y)
    gm = mixture.GMM(n_components=i, covariance_type='tied')
    gm.fit(X_new)
    ypred = gm.predict(icanew)
    homo.append(metrics.homogeneity_score(y, ypred))
    comp.append(metrics.completeness_score(y, ypred))
    v_mea.append(metrics.v_measure_score(y, ypred))
    sil.append(metrics.silhouette_score(icanew, ypred,metric='euclidean'))
    man.append(metrics.silhouette_score(icanew, ypred, metric='manhattan'))
