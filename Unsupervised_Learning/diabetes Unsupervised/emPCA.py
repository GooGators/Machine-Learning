...
X_new = decomposition.pca.PCA(n_components=6).fit_transform(X)
...
for i in range(2, numPoints):
    ...
    sil.append(metrics.silhouette_score(X_new, gm.predict(X_new), metric='euclidean'))
    man.append(metrics.silhouette_score(X_new, gm.predict(X_new), metric='manhattan'))
