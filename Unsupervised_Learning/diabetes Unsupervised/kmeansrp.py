...
for i in range(2, numPoints):
    ...
    euc.append(metrics.silhouette_score(projected , km.labels_, metric='euclidean'))
    man.append(metrics.silhouette_score(projected , km.labels_, metric='manhattan'))
...
km_train = km.fit(projected_data)
clus = km_train.predict(projected_data)
